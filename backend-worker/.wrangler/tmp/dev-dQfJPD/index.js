...
// .wrangler/tmp/bundle-biysCo/checked-fetch.js
...
// .wrangler/tmp/bundle-biysCo/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-biysCo/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-biysCo/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
__name(CSSQueryHandler, "CSSQueryHandler");
...
var unquote = /* @__PURE__ */ __name((text) => {
  ...
  return text.replace(ESCAPE_REGEXP, (match2) => {
    return match2[1];
  });
}, "unquote");
...
__name(CdpPreloadScript, "CdpPreloadScript");
...
var BaseRepository = class {
  ...
  async create(data) {
    const columns = this.writableColumns.filter((column) => data[column] !== void 0);
    const placeholders = columns.map(() => "?").join(", ");
    const values = columns.map((column) => normalizeValue2(data[column]));
    const row = await this.db.prepare(`INSERT INTO ${this.table} (${columns.join(", ")}) VALUES (${placeholders}) RETURNING *`).bind(...values).first();
    if (!row)
      throw new Error(`Failed to create ${this.table} row`);
    return row;
  }
  async update(id, data) {
    ...
    if (!columns.length) {
      ...
      if (!existing)
        throw new Error(`${this.table} row not found`);
      return existing;
    }
...
__name(BaseRepository, "BaseRepository");
...
__name(CoursesRepository, "CoursesRepository");
...
__name(ModulesRepository, "ModulesRepository");
...
__name(ReviewerRepository, "ReviewerRepository");
...
__name(WorkflowRepository, "WorkflowRepository");
...
function crudRoute(table, columns, filters, adminWrite = true) {
  ...
  app2.post("/", async (c) => {
    if (adminWrite && !isAcademicAdmin(c.get("user")))
...
api.post("/reviewer-comments/", async (c) => {
  if (!isReviewerOrAdmin(c.get("user")))
    return c.json({ detail: "Permission denied." }, 403);
...
api.post("/published-curricula/publish/", async (c) => {
  ...
  const count = await c.env.DB.prepare("SELECT count(*) AS n FROM courses c JOIN semesters s ON s.id = c.semester_id WHERE s.department_id = ? AND s.academic_year_id = ? AND c.status IN ('APPROVED','PUBLISHED')").bind(body.department, body.academic_year).first();
...
async function syncCourse(db, courseId, data) {
  ...
  if (data.modules) {
    await syncChildren(db, "modules", "course_id", courseId, data.modules, ["number", "title", "contact_hours", "content"], void 0, async (module, row) => {
...
async function syncChildren(db, table, parentColumn, parentId, items, fields, mapItem = (item, _i) => item, afterUpsert) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const values = columns.map((field) => field === parentColumn ? parentId : normalizeValue2(item[field]));
    let row;
    if (item.id) {
  ...
  async queue(batch, env, ctx) {
    for (const message of batch.messages) {
      ...
      try {
        ...
        let browser;
        try {
          browser = await puppeteer_cloudflare_default.launch(env.BROWSER);
        } catch (launchErr) {
        ...
        try {
          await page.waitForSelector('main[data-fonts-loaded="true"]', { timeout: 15e3 });
        } catch (fontErr) {
